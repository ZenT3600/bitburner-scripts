...
export async function generateCommand(ns, target) {
  if (ns.serverExists(target)) {
    let path = await generatePathRaw(ns, target)
...
export async function main(ns) {
    ...
    let command = await generateCommand(ns, target)
    if (command) {
